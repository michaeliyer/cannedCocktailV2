...
app.get("/inventory-summary", (req, res) => {
  const query = `
    SELECT 
      ...
      v.sku,
      v.unit_price,
      v.units_in_stock + COALESCE(v.units_sold, 0) AS original_stock,
      COALESCE(v.units_sold, 0) AS sold_units,
      v.units_in_stock AS current_stock,
...
app.get("/product-report", (req, res) => {
  ...
  db.all(query, params, (err, rows) => {
    if (err) {
      console.error("Error fetching product report:", err.message);
      return res.status(500).json({ error: err.message });
    }
